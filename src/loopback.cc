...
AlsaLoopback::AlsaLoopback(std::string &&device) {
  int err;
  ...
  if ((err = snd_pcm_set_params(playback_handle, format,
                                SND_PCM_ACCESS_RW_INTERLEAVED, nchannels, rate,
                                0, 50000)) < 0) {
...
void AlsaLoopback::push(uint8_t buf[AlsaLoopback::bufsize]) {
  int err;
