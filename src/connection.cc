...
const char *DROIDCAM_CONNECT_ERROR = "Connect failed, please try again.\n"
                                     "Check IP and Port.\n"
...
SOCKET Connect(const char *ip, int port, const char **errormsg) {
  ...
  if (!(errno == EAGAIN || errno == EWOULDBLOCK || errno == EINPROGRESS) ||
      (select(sock + 1, NULL, &set, NULL, &timeout) <= 0)) {
...
void disconnect(SOCKET s) { close(s); }
...
SOCKET accept_connection(int port) {
  ...
  while ((client = accept(wifiServerSocket, NULL, NULL)) == INVALID_SOCKET) {
